 ...
 */
@RestController
@RequestMapping("/system/configsk")
@PreAuthorize("@ss.hasRole('system:configsk:*')")
public class SysConfigSkController extends BaseController {
    @Autowired
    ...
    public TableDataInfo list(SysConfig config) {
        ...
        if (config.getIsSys() != null && "1".equals(config.getIsSys())) {
            ...
            SysConfig config_ = new SysConfig();
            List<SysConfig> list_ = configService.selectConfigListSk(config_);
            return getDataTable(list_);
        } else {
